...
?>
 ...
 $q=$bd->query("SELECT * FROM etudiant INNER JOIN groupe ON groupe.idG = etudiant.idG INNER JOIN section ON section.id_section = groupe.id_section INNER JOIN niveau_de_formation ON niveau_de_formation.id_n = section.id_n WHERE identifiant='". $_GET['ide']."'");
$t=$q->fetch();
...
if (isset($_POST['submit'])) {
	...
	$q=$bd->query("INSERT INTO `periode_examen`(`id_p`, `heure_debut`, `heure_fin`, `date_p`) VALUES (NULL,'".$_POST['hd']."','".$hj."','".$_POST['d']."')");

$q=$bd->query("UPDATE `module` SET `id_p` = LAST_INSERT_ID() WHERE `module`.`id_module` = '".$_POST['m']."'");

}
?>
<a href="supp1.php?idn=<?php echo $t['id_n']."&ide=".$_GET['ide'];?> ">supprimer tout</a>
<?php
